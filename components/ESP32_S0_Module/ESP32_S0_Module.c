...
#ifndef ESP32_S0_Module_DBG  
#define ESP32_S0_Module_DBG  5	// 5 is default
#endif 
...
/**
 * -------------------------------------------------------------------------------------------------
 *  DName: ESP32_S0_ActiveResourcesDataA (for WebIf import SCDE Generation 1)
...
const WebIf_ActiveResourcesDataA_t ESP32_S0_ActiveResourcesDataA_forWebIf[] = {
  // Switch-Feature Control
  { 0b00000000000000000000000000001010, 0b0000000000011110, 0b0000000000000001,   0,    1, 0b00000000, "/\xf1\x00\x08/SwITCH\x00 SwITCHX.htm"},
  // RENAMED Switch-Feature Control
  { 0b00000000000000000000000000001010, 0b0000000000011110, 0b0000000000000001,   0,    1, 0b00000000, "/\xf3\x00\x08\x00 SwITCHX.htm"},
  ...
  { 0b00000000000000000000000000000110, 0b0000000000010001, 0b0000000000001000,   0,    1, 0b00000000, "/\xf1\x00\x08"},
  // RENAMED Switch-Feature Control - Home Automation SwITCH:// access Resources
  { 0b00000000000000000000000000000110, 0b0000000000010001, 0b0000000000001000,   0,    1, 0b00000000, "/\xf3\x00\x08"},
...
/**
 * -------------------------------------------------------------------------------------------------
 *  DName: ESP32_S0_ActiveResourcesDataB (for WebIf import SCDE Generation 1)
 *  Desc: Resource-Content of active Directory - PART B (Procedure-Call-Data-Rows) 
 *  Data: WebIf_ActiveResourcesDataA_t from WebIf_Module.h
 * -------------------------------------------------------------------------------------------------
 */
// CgiFucID=(No.<<16)+AllowedSchemeBF |      cgi            |     cgi_data
const WebIf_ActiveResourcesDataB_t ESP32_S0_ActiveResourcesDataB_forWebIf[] =  {

  // S0-Feature Control
  {(1<<16) +	0b0000000000000010,	NULL,NULL}, //EspFsTemplate_cgi,	S0_tpl		},
  {(1<<16) +	0b0000000000000100,	NULL,NULL}, //S0_cgi,			NULL		},
  {(1<<16) +	0b0000000000001000,	NULL,NULL}, //S0_jso,			NULL		},
  {(1<<16) +	0b0000000000010001,	NULL,NULL}, //S0_set,			NULL		},

  // Multi S0(s)-Feature Control
  {(2<<16) +	0b0000000000000010,	NULL,NULL}, //EspFsTemplate_cgi,	S0_tpl		},
  {(2<<16) +	0b0000000000001000,	NULL,NULL}  //S0_jso	,		NULL		}
 ...
 * -------------------------------------------------------------------------------------------------
 */
ProvidedByModule_t ESP32_S0_ProvidedByModule =  { // A-Z order
  "ESP32_S0"				// Type-Name of module -> on Linux libfilename.so !
  ,8					// size of Type-Name
  ...
  ,ESP32_S0_State			// State
  ,NULL					// Sub
  ,ESP32_S0_Undefine			// Undefine
  ,NULL					// Write
  ,NULL					// FnProvided
  ,sizeof(ESP32_S0_Definition_t)	// Modul specific Size (Common_Definition_t + X)
};
...
ESP32_S0_Attribute(Common_Definition_t* p_entry_definition,
	 const String_t attr_command,
	 const String_t attr_name,
	 const String_t attr_value)
{
  // make common ptr to modul specific ptr
  ESP32_S0_Definition_t* p_entry_esp32_s0_definition =
		  (ESP32_S0_Definition_t*) p_entry_definition;
...
// -------------------------------------------------------------------------------------------------
  ...
  #if ESP32_S0_Module_DBG >= 5
  SCDEFn_at_ESP32_S0_M->Log3Fn(p_entry_esp32_s0_definition->common.name,
	p_entry_esp32_s0_definition->common.nameLen,
	5,
	"Attribute Fn (Module '%.*s') is called with args: "
	"attr_command '%.*s' attr_name '%.*s' attr_value '%.*s'",
	p_entry_esp32_s0_definition->common.module->provided->typeNameLen,
	p_entry_esp32_s0_definition->common.module->provided->typeName,attr_command.len,
	attr_command.p_char,
  	attr_name.len,
	attr_name.p_char,
	attr_value.len,
...
//	&ESP32Control_Definition->WebIf_Provided;
  ...
  ESP32_S0_Definition->WebIf_Provided.WebIf_FnProvided =
	NULL;//(WebIf_FnProvided_t *) SCDEFn_at_ESP32_S0_M->GetFnprovided("WebIf");
 ...
 // Providing data for WebIf? Initialise data provided for WebIf
  if (ESP32_S0_Definition->WebIf_Provided.WebIf_FnProvided) {
	...
	SCDEFn_at_ESP32_S0_M->Log3Fn(Common_Definition->name
		,Common_Definition->nameLen
		,1
		,"Could not enable WebIf support for '%.*s'. Type '%.*s' detects Type 'WebIf' is NOT loaded!"
		,ESP32_S0_Definition->common.nameLen
		,ESP32_S0_Definition->common.name
		,ESP32_S0_Definition->common.module->provided->typeNameLen
		,ESP32_S0_Definition->common.module->provided->typeName);
	}
...
// ------------------------------------------------------------------------------------------------
  ...
  parsedKVInputArgs_t *parsedKVInput = 
	SCDEFn_at_ESP32_S0_M->ParseKVInputArgsFn(ESP32_S0_SET_IK_Number_of_keys	// Num Implementated KEYs MAX
...
ESP32_S0_Initialize(SCDERoot_t* SCDERootptr)
{
  ...
  SCDERoot_at_ESP32_S0_M = SCDERootptr;

  // make locally available from data-root: SCDEFn (Functions / callbacks) for faster operation
  SCDEFn_at_ESP32_S0_M = SCDERootptr->SCDEFn;

  SCDEFn_at_ESP32_S0_M->Log3Fn(ESP32_S0_ProvidedByModule.typeName
...
// ------------------------------------------------------------------------------------------------
  ...
  parsedKVInputArgs_t *parsedKVInput = 
	SCDEFn_at_ESP32_S0_M->ParseKVInputArgsFn(ESP32_S0_SET_IK_Number_of_keys	// Num Implementated KEYs MAX for Set Fn
 ...
 *  Para: ESP32_S0_Definition_t *ESP32_S0_Definition -> Definition that gets an status update
 ...
 */
strTextMultiple_t*
ESP32_S0_State(Common_Definition_t *Common_Definition
	,time_t readingTiSt
	,uint8_t *readingName
	...
	,size_t readingValueLen)
{
  ...
  strText_t strText =
  	SCDEFn_at_ESP32_S0_M->FmtDateTimeFn(readingTiSt);
...
// 4. Step: Passed. Store the new values
  ...
  if (parsedKVInput->keysFoundBF & ( (1 << ESP32_S0_SET_CH1_LCTRL_MODE)
	...
	SCDEFn_at_ESP32_S0_M->HexDumpOutFn ("dump"
		,pS0_CNT_CONF_REG
		, sizeof(ESP32_S0_CNT_CONF_REG_t) );
	# endif
  }
	...
	,uint64_t affectedReadings)
{
  ...
  SCDEFn_at_ESP32_S0_M->readingsBeginUpdateFn((Common_Definition_t*) ESP32_S0_Definition);
...
//>Add Readings Bit 00 -----------------------------------------------------------------------------
  ...
  if (affectedReadings & ESP32_S0_R_ATT) {
	...
	SCDEFn_at_ESP32_S0_M->readingsBulkUpdateFn((Common_Definition_t*) ESP32_S0_Definition
		,readingNameText
		,readingNameTextLen
		,readingValueText
		,readingValueTextLen);
...
//--------------------------------------------------------------------------------------------------
  ...
  SCDEFn_at_ESP32_S0_M->readingsEndUpdateFn((Common_Definition_t*) ESP32_S0_Definition, true);
...
// -------------------------------------------------------------------------------------------------
  ...
  ESP32_S0_CNT_CONF_REG_t *pESP32_S0_CNT_CONF_REG = (ESP32_S0_CNT_CONF_REG_t*) 0x3FF570B0;
	...
	SCDEFn_at_ESP32_S0_M->HexDumpOutFn ("verify:"
