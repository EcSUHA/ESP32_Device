 ...
 *       Resource-Configuration: Methods SET || GET ; Scheme SwITCH ; Path opt. with UID ; Mime .set|-/- 
 *       Used Header-Fields:
...
// For: SwITCH_set RPC, Finite State Machine processing State - Stage 2 
enum SwITCH_set_state
  {					// #XX for debugging
...
ESP32_SwITCH_set(WebIf_HTTPDConnSlotData_t *conn) {
...
// int stage2_scde_errno = (enum http_errno) 0; //enum?
  ...
  enum SwITCH_set_state S2_State = (enum SwITCH_set_state) S2_Statex;
...
//--------------------------------------------------------------------------------------------------
	...
	case S2S_set_Action_S1_Parse_Body_Data:
	break;
...
//--------------------------------------------------------------------------------------------------
	...
	if (SCDED_AuthCheck(conn, AUTH_SET_RESSOURCE)) {
...
/*
	...
	conn->parser_http_errno = SwITCH_CmdParser(conn
			,Args
		      //,HdrFldsLinkedList?
		      //,char* Data //BdyData
			,ADID
