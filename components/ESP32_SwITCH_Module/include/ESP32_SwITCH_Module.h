...
// Universal Switch-Feature-Data -> in FlashNoMirror part
typedef struct {
  // no data in this project
} SwITCH_Feat_Cfg_Flash_No_Mirror_t;

// Universal Switch-Feature-Data -> in RamFlashMirrored part
typedef struct {
  //--- Internals - Block 1 ---
//uint32_t Capabilities;		// =CAPS, capabilities of Devices S0 Feature - HARDCODED!
  ...
  uint8_t unused3;
  uint8_t unused4;
} SwITCH_Feat_Cfg_Ram_Flash_Mirrored_t;

// Universal Switch-Feature-Data -> in RamRtcMirrored part
typedef struct {
  uint32_t RB01_Val;			// val -> switch value = 1 byte
} SwITCH_Feat_Cfg_Ram_Rtc_Mirrored_t;

// Universal Switch-Feature-Data -> in RamNoMirror part
typedef struct {
  int32_t IB01_X_TiSt;			// tist -> for Internals-Block-01 sync
  int32_t RB01_X_TiSt;			// tist -> for Readings-Block-01 sync
} SwITCH_Feat_Cfg_Ram_No_Mirror_t;
...
Entry_String_t* ESP32_SwITCH_Attribute (Common_Definition_t* p_entry_definition, const String_t attr_command, const String_t attr_name, const String_t attr_value);

strTextMultiple_t* ESP32_SwITCH_Define(Common_Definition_t *Common_Definition);


Entry_String_t*    ESP32_SwITCH_GetV2(Entry_Definition_t* p_definition, char **argv ,int argc);

strTextMultiple_t* ESP32_SwITCH_Get(Common_Definition_t* Common_Definition, uint8_t *getArgs, size_t getArgsLen);
...
Entry_String_t*    ESP32_SwITCH_State(Common_Definition_t *Common_Definition, const time_t stateTiSt, const String_t stateNameString, const String_t stateValueString, const String_t stateMimeString);
