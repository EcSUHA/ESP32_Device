...
#include "ESP32_SwITCH_WEBIF_get.h"

/*
 *--------------------------------------------------------------------------------------------------
 *FName: ESP32_SwITCH_WEBIF_get
...
int
ESP32_SwITCH_WEBIF_get(WebIf_HTTPDConnSlotData_t* conn)
{
...
//---------------------------------------------------------------------------------------------------
  ...
  strTextMultiple_t* respMsg = NULL;
...
// ------------------------------------------------------------------------------------------------
	...
	respMsg = ESP32_SwITCH_Set(conn->p_entry_active_dir_matching_definition
		,setArgs
                ,setArgsLen);
...
//--------------------------------------------------------------------------------------------------
	...
	if (respMsg) {
		...
		if (respMsg->strText) free(respMsg->strText);
		free(respMsg);
	}
