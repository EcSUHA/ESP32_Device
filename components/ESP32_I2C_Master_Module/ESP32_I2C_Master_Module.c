...
ProvidedByModule_t ESP32_I2C_Master_ProvidedByModule = {   // A-Z order
  ...
  ,&ESP32_I2C_Master_Fn			// FnProvided
...
ESP32_I2C_Master_Attribute(Common_Definition_t* p_entry_definition,
	 const String_t attr_command,
	 const String_t attr_name,
	 const String_t attr_value)
{
  // make common ptr to modul specific ptr
  ESP32_I2C_Master_Definition_t* p_entry_esp32_i2c_master_definition =
		  (ESP32_I2C_Master_Definition_t*) p_entry_definition;
...
// -------------------------------------------------------------------------------------------------
  ...
  SCDEFn_at_ESP32_I2C_Master_M->Log3Fn(p_entry_esp32_i2c_master_definition->common.name,
	p_entry_esp32_i2c_master_definition->common.nameLen,
	5,
	"Attribute Fn (Module '%.*s') is called with args: "
	"attr_command '%.*s' attr_name '%.*s' attr_value '%.*s'",
	p_entry_esp32_i2c_master_definition->common.module->provided->typeNameLen,
	p_entry_esp32_i2c_master_definition->common.module->provided->typeName,attr_command.len,
	attr_command.p_char,
  	attr_name.len,
	attr_name.p_char,
	attr_value.len,
...
// -------------------------------------------------------------------------------------------------
  ...
  #if ESP32_I2C_Master_Module_DBG >= 5
  SCDEFn_at_ESP32_I2C_Master_M->Log3Fn(Common_Definition->name
...
//	&ESP32Control_Definition->WebIf_Provided;
  ...
  ESP32_I2C_Master_Definition->WebIf_Provided.WebIf_FnProvided =
	NULL;//(WebIf_FnProvided_t *) SCDEFn_at_ESP32_I2C_Master_M->GetFnProvidedByModule("WebIf");
...
// ------------------------------------------------------------------------------------------------
  ...
  parsedKVInputArgs_t *parsedKVInput = 
	SCDEFn_at_ESP32_I2C_Master_M->ParseKVInputArgsFn(ESP32_I2C_Master_SET_NUMBER_OF_IK		// Num Implementated KEYs MAX
...
// -------------------------------------------------------------------------------------------------
  #if ESP32_I2C_Master_Module_DBG >= 5
  SCDEFn_at_ESP32_I2C_Master_M->Log3Fn(
	...
	,ESP32_I2C_Master_Definition_Stage1->common.nameLen
	,ESP32_I2C_Master_Definition_Stage1->common.name
	,ESP32_I2C_Master_Definition_Stage1->common.module->provided->typeNameLen
	,ESP32_I2C_Master_Definition_Stage1->common.module->provided->typeName
...
/*
  ...
  ESP32_I2C_Master_CNT_CONF_REG_t *pESP32_I2C_Master_CNT_CONF_REG = (ESP32_I2C_Master_CNT_CONF_REG_t*) 0x3FF57000;
	SCDEFn_at_ESP32_I2C_Master_M->HexDumpOutFn ("verify:"
...
ESP32_I2C_Master_Initialize(SCDERoot_t* SCDERootptr)
{
  ...
  SCDERoot_at_ESP32_I2C_Master_M = SCDERootptr;

  // make locally available from data-root: SCDEFn (Functions / callbacks) for faster operation
  SCDEFn_at_ESP32_I2C_Master_M = SCDERootptr->SCDEFn;

  SCDEFn_at_ESP32_I2C_Master_M->Log3Fn(ESP32_I2C_Master_ProvidedByModule.typeName
...
// ------------------------------------------------------------------------------------------------
  ...
  parsedKVInputArgs_t *parsedKVInput = 
	SCDEFn_at_ESP32_I2C_Master_M->ParseKVInputArgsFn(ESP32_I2C_Master_SET_NUMBER_OF_IK	// Num Implementated KEYs MAX for Set Fn
...
// -------------------------------------------------------------------------------------------------
  ...
  i2c_cmd_handle_t cmd = (i2c_cmd_handle_t) data.p_char;
...
/*
  ...
  if (parsedKVInput->keysFoundBF & ( (1 << ESP32_I2C_Master_SET_TIMER)
	...
	printf("|ESP32_PWM_Channel_s addr=%p>"
		,pPWM_Channel);
	SCDEFn_at_ESP32_I2C_Master_M->HexDumpOutFn ("dump"
		,pPWM_Channel
...
/*
 affectedReadings = 0xfffffffffffffff;
  // do we have updates?
  if (affectedReadings) {
  SCDEFn_at_ESP32_I2C_Master_M->readingsBeginUpdateFn((Common_Definition_t*) ESP32_I2C_Master_Definition);
  }
  // we have no updates
  else return false;
// -----------------------------------------------------------------------------------------------
...
//>Add Readings Bit 00 -----------------------------------------------------------------------------
  ...
  if (affectedReadings & ESP32_I2C_Master_R_DUTY) {
	...
	readingValueTextLen = asprintf((char **) &readingValueText
		,"%u"
		,pPWM_Channel->DUTY);
	SCDEFn_at_ESP32_I2C_Master_M->readingsBulkUpdateFn((Common_Definition_t*) ESP32_I2C_Master_Definition
		,readingNameText
		,readingNameTextLen
		,readingValueText
		,readingValueTextLen);
...
//  SCDEFn_at_ESP32_I2C_Master_M->readingsEndUpdateFn((Common_Definition_t*) ESP32_I2C_Master_Definition);
